...
- [ ] T005 [US5] `dock/item.html`: `dock-active`, `aria-current="page"` on the link only, `type="button"` on the button branch, `<c-icon … aria-hidden="true">`, label in `dock-label`, the toggle's `aria-label` only when `label` is given. Tests for scenarios 4–6 and an icon-only item named by the caller's `aria-label` (FR-003, FR-005, FR-022).
...
- [ ] T008 [US1] `menu/item.html`: link or `<button type="button">`, `active` (`menu-active`, and `aria-current="page"` on a link), `disabled` (`menu-disabled` on the `<li>`, `aria-disabled="true"` and no `href` on a link, native `disabled` on a button), icon through `<c-icon>`, attributes on the `<li>`. Tests for scenarios 4–5 and several active items each rendered as given (FR-003, FR-005, FR-012).
...
- [ ] T011 [US2] `navbar.html`: `<nav class="navbar …">` with `start`, `center`, `end` named slots each emitted only when given, default slot directly inside the root, merged `class`, `aria-label` overridable with the translated default `Main`. Tests for scenarios 1–5 in a new `tests/test_navbar.py` (FR-006, FR-014).
...
- [ ] T014 [US4] `tabs/tab.html` link and button shapes: link with `tab tab-active` and `aria-current="page"`, disabled with `tab-disabled`, `aria-disabled="true"` and no `href`, button shape `<button type="button" role="tab">` with `aria-selected` and native `disabled`. Tests for scenarios 4, 5 and 7 (FR-005, FR-019).
- [ ] T015 [US4] `tabs/tab.html` radio shape: `<input type="radio" class="tab" name aria-label>` followed by its `tab-content` panel, `checked` on `active`, `disabled`. Tests for scenario 6, rendering a three-tab set (FR-019, FR-020).
...
- [ ] T019 [US7] `megamenu/index.html`: required `id`, the small-screen toggle through `<c-button class="sm:hidden" popovertarget>`, `<nav id popover class="megamenu max-sm:megamenu-vertical …">` with the `megamenu-active` span, `wide`, `full`, `size`, `aria-label` overridable with the translated default `Site`. Tests for scenarios 1, 2, 4 and 5 in a new `tests/test_megamenu.py` (FR-003, FR-006, FR-025, FR-026).
...
- [ ] T021 [US7] Gallery annotations (description mentions daisyUI's ten-item limit), the default `@slot` example three items holding menus, README and CHANGELOG `Added` (FR-007, FR-008, FR-009).

**Checkpoint**: batch 3 green.

## Phase 8: Verification at the walkthrough (orchestrator)

- [ ] T022 Run axe-core and the scripted keyboard walk against every gallery entry in the group on the running demo (research R7) and record the result in the pull request's test evidence (SC-004).
