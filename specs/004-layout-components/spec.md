...
4. **Given** `placement="end"`, **When** the drawer renders, **Then** its root carries `drawer-end` and the sidebar opens from the other side.
...
2. **Given** `horizontal="sm"`, **When** it renders, **Then** the root carries `sm:footer-horizontal`. **Given** `vertical`, it carries `footer-vertical`. **Given** `placement="center"`, it carries `footer-center`.
...
3. **Given** `placement="start"` or `placement="end"`, **When** it renders, **Then** it carries `divider-start` or `divider-end`.
...
2. **Given** an item with `placement="bottom start"`, **When** it renders, **Then** it carries `indicator-bottom` and `indicator-start`. **Given** no placement, it carries neither and takes daisyUI's default top-end corner.
...
5. **Given** a stack with three children and `placement="top"`, **When** it renders, **Then** the root carries `stack` and `stack-top`, and every child stays available to assistive technology.
...
- **An enumerated attribute given a value daisyUI does not define** (`variant="pink"`,
  `shape="blob"`, `placement="left"`): no modifier class is emitted for it, the component renders
...
- **FR-011**: `drawer` MUST accept `open` (daisyUI's `drawer-open`, per FR-004) and `placement="end"` (`drawer-end`). (US1)
...
- **FR-013**: `footer` MUST render a `<footer>` element carrying `footer`, and accept `horizontal` and `vertical` (per FR-004) and `placement="center"`. (US2)
...
- **FR-015**: `divider` MUST map `horizontal` to `divider-horizontal` and `vertical` to `divider-vertical` (per FR-004), `variant` to daisyUI's divider colours, and `placement` to `divider-start` or `divider-end`. Its label MUST come from the default slot or a `text` attribute. (US3)
...
- **FR-019**: `indicator.item` MUST carry `indicator-item` and accept `placement` with up to one horizontal placement (`start`, `center`, `end`) and one vertical placement (`top`, `middle`, `bottom`). (US5)
...
- **FR-022**: `stack` MUST carry `stack` and accept `placement` (`top`, `bottom`, `start`, `end`). (US7)
...
- **SC-003**: Every variant, direction, placement, shape and open state listed in FR-008 appears in the component gallery, with none missing.
...
- Q: How are direction, placement and open state named across components? → A: Direction is two boolean attributes named as daisyUI names the modifiers (`horizontal`, `vertical`), each also taking a breakpoint name for the responsive form, because daisyUI's own responsive pattern puts both on one element. Placement is `placement`, daisyUI's own name for that modifier category, used by every component that has placement modifiers. The drawer's `drawer-open` is a boolean `open` that also takes a breakpoint.
