...
- Discovery reuses the gallery's own scanner, `django_cotton_gallery.core.catalog.scanner.scan`, pointed at the package's `cotton/` directory through a `CatalogConfig`. That keeps "what is a component and what is it called" identical to the gallery's, including the `<dir>/index.html` convention, and a new template is picked up with no change to the test (FR-004).
...
- The catalog test is parametrised by component path, so a failure names the component in the test id. Its assertion message lists every blocking finding as `<component> L<line> <rule>: <message>` (FR-003).
- Tests against scratch sources prove the check can fail: an undeclared `@prop`, an unknown component, a hints-only template that passes, and a template added to a scratch `cotton/` directory that is discovered.

### The annotation check (US7) — `tests/test_gallery_annotations.py`

Reads every template under the package's `cotton/` directory (by `rglob("*.html")`, the same set the scanner yields for this package) and checks:

- exactly one `{# @description … #}` (FR-015)
- when the template renders the default slot — a `{{ slot }}` expression, filters allowed, outside `{# … #}` and `{% comment %}` — at least one default `{# @slot … #}`, meaning `@slot` not followed by `:` (FR-016)
...
Each annotation story is proven by the linter's own output for its templates (red on main, clean after), plus a whitespace-normalised comparison of every component's rendered output before and after (FR-014). Those comparisons are run, recorded as evidence and not committed: once the checks land, the lint and annotation rules are guarded by the suite, and the existing render tests guard the markup.
