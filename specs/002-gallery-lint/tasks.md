...
- [ ] T007 [P] [US5] Annotate `breadcrumbs/index.html` and `breadcrumbs/item.html`, each `@description` saying how it relates to the other (FR-011).
- [ ] T008 [P] [US5] Annotate `dock/index.html` and `dock/item.html`, same relationship rule (FR-011).
- [ ] T009 [P] [US5] Annotate `avatar/index.html` and `avatar/group.html`. `size_options` and `space_options` are documented as they behave today and are not renamed or removed (FR-011, FR-014).

## Phase 5: US6 — Mockup components are fully annotated (P3)

- [ ] T010 [P] [US6] Annotate `mockup/browser.html`, `mockup/window.html` and `mockup/phone.html` (FR-012).
- [ ] T011 [P] [US6] Annotate `mockup/code/index.html` and `mockup/code/line.html` (FR-012).
...
- [ ] T017 [US8] Link `CONTRIBUTING.md` from the README and add the CHANGELOG `[Unreleased]` entry: every component is documented in the gallery, and the test suite enforces it (FR-019, FR-020).
