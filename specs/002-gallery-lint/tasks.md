...
- [x] TC03 [US3] `form.field`: the `help-text` annotation and comment name the slot that actually renders (`help_text`); the `@slot` and `wrapper-class` descriptions cover checkbox, radio, toggle and file controls.
- [x] TC04 [US4] `dropdown`'s `full` and `class`, `alert`'s `class` and `modal`'s `size` descriptions say everything the attribute does today; `modal`'s `id` gives the gallery an example id matching its `@trigger`.
