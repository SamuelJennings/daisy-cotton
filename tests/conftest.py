...
def render() -> Callable[..., str]:
    ...
    no component at all — which reads as a component that produced nothing
    rather than as a test that never ran one. Running the compiler first is
    exactly what the loader does with a file on disk, and it lets a test state
    the markup it is about in its own body instead of keeping a fixture
    template per case somewhere else.
    """
